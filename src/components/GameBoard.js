...
class GameBoard extends Component {
  ...
  startPositions = [[1, 11, 20, 30, 39, 49], [10, 19, 29, 38, 48, 57]];
  ...
  findPossibleMoves = (monster, originTile) => {
    ...
    console.log(threeTiles);

    if (monster.evo_level === 1) {
      console.log(oneTile.concat(twoTiles).concat(threeTiles));
      return oneTile.concat(twoTiles).concat(threeTiles);
    }

    const roll =
      this.props.currentTurn === 1 && this.props.p1Move
        ? this.props.p1Move
        : this.props.currentTurn === 2
        ? this.props.p2Move
        : null;
    ...
    if (monster.evo_level === 2 && roll > 3) {
      ...
      console.log(oneTile.concat(twoTiles));
    ...
    if ((monster.evo_level === 3 || monster.evo_level === 6) && roll > 3) {
      ...
      (monster.evo_level === 3 || monster.evo_level === 6) &&
      roll < 4
    ) {
      console.log(oneTile);
      return oneTile;
  ...
  withinMoveRange = toTile => {
    ...
    // console.log(possibleTiles.includes(toTile));
    return possibleTiles.includes(toTile);
    // if (possibleTiles !== undefined && possibleTiles.includes(toTile)) {
    //   return possibleTiles.includes(toTile);
    // }
  };
  ...
  getAllSurroundingEnemies = teamId => {
    ...
    team.forEach(monster => {
      opposing.forEach(enemy => {
        if (this.withinOneTile(monster.tile, enemy.tile)) {
  ...
  checkWin = () => {
    if (
      this.state.team1Roster.length === 0 ||
      this.state.team2Roster.length === 0
    ) {
  ...
  randomizeTiles = () => {
    const tileBg = {};
    for (let i = 1; i <= 57; i++) {
      tileBg[i] = this.state.groundTiles[
        Math.floor(Math.random() * this.state.groundTiles.length)
      ];
    }
  ...
  render() {
    ...
    return (
      <div className="board">
        <ul id="hexGrid">{this.renderBoard()}</ul>
      </div>
    );
  }
}
