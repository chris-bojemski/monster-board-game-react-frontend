    ...
    return this.state.assignments
      ...
      .map(assignment => {
        return this.state.monsters.find(
          monster => monster.id === assignment.monster_id
        );
      });
  };
  ...
  selectTeam = teamId => {
    if (this.state.team1 === null) {
      this.setState(
        {
          team1: teamId
          //team1Roster: this.findTeamMonsters(teamId)
        },
        () => this.props.loadTeamFromApi(this.findTeamMonsters(teamId))
      );
    } else if (this.state.team2 === null) {
      this.setState(
        { team2: teamId, team2Roster: this.findTeamMonsters(teamId) },
        () => {
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        <a href="https://github.com/astrosquid/monster-board-game-react-frontend">
          <img
            className="logo"
...
const mapDispatchToProps = dispatch => ({
  loadTeamFromApi: apiData =>
    dispatch({
      type: "LOAD_TEAM_FROM_API",
      payload: apiData
    })
});
const exposeTeam1Roster = connect(
  mapStateToProps,
  mapDispatchToProps
);
const connectedApp = exposeTeam1Roster(App);
